...
El comando para generar el hash BCrypt de la contraseña depende de si estás en desarrollo (`docker-compose.dev.yml`, imagen con Maven) o en producción (`docker-compose.prod.yml`, imagen runtime sin Maven):

**a) Genera el hash BCrypt de la contraseña que quieras usar:**

- En desarrollo (con la app corriendo vía `docker-compose.dev.yml`):
...
- En producción (con la app corriendo vía `docker-compose.prod.yml`): la imagen runtime (`eclipse-temurin:21-jre`) no tiene Maven ni `~/.m2`, así que no se puede armar el classpath buscando jars ahí. En su lugar, se usa el `PropertiesLauncher` de Spring Boot contra el `app.jar` ya empaquetado dentro del contenedor:
...
Cualquiera de los dos comandos imprime un hash como `$2a$10$....` — cópialo.
...
- En producción (el contenedor de base de datos se sigue llamando `apivapati_java_db`, usa el `DB_PASSWORD` real del `.env` del servidor):
...
(en producción, reemplaza `localhost:8080` por el host/puerto real del servidor)
...
Una vez que el código está listo, la imagen se construye y publica manualmente (no hay CI/CD automático en este flujo):
...
Reemplaza `<usuario-dockerhub>` por tu usuario de Docker Hub y `<tag>` por la versión que corresponda (por ejemplo `latest` o `1.0.0`).

### 7. Desplegar en un servidor

El servidor de despliegue **solo necesita** `docker-compose.prod.yml` y un `.env` con los valores reales de producción — no necesita el resto del repositorio, ya que la app corre desde la imagen publicada en Docker Hub.

1. Copia `docker-compose.prod.yml` y tu `.env` de producción (con `DB_PASSWORD`, `JWT_SECRET`, `CORS_ALLOWED_ORIGINS`, `IMAGE_NAME`, `IMAGE_TAG` y los puertos que correspondan) al servidor.
2. Descarga la imagen publicada:
...
Esto levanta `apivapati_java_db` (SQL Server) y `apivapati_java_app` (la imagen publicada), y la app queda accesible en el puerto configurado (`APP_PORT`). El reverse proxy, HTTPS y dominio quedan a cargo de la configuración del servidor.

> ⚠️ `DB_PASSWORD` y `JWT_SECRET` son obligatorios en producción. Si el `.env` no los define, `application-docker.properties` cae en los valores de desarrollo por defecto (`TuContrasenaSegura!` / `your-512-bit-secret-key-should-be-long-and-random`), que **no son seguros para producción**.
